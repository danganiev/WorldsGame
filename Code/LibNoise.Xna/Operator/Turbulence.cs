...
namespace LibNoise.Xna.Operator
{
	...
	public class Turbulence : ModuleBase
	{
		...
		public override double GetValue(double x, double y, double z)
		{
			...
			double xd = x + (_xDistort.GetValue(x + X0, y + Y0, z + Z0) * _power);
			double yd = y + (_yDistort.GetValue(x + X1, y + Y1, z + Z1) * _power);
			double zd = z + (_zDistort.GetValue(x + X2, y + Y2, z + Z2) * _power);
