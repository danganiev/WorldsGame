...
{
    internal class BlockDebugger : PlayerTool
    {
        internal BlockDebugger(Player player)
            : base(player)
        {
        }
        ...
        {
            PositionedBlock b = Player.CurrentSelection;
            Vector3i pos = b.Position;
            Chunk chunk = Player.World.GetChunk(pos);
            BlockType block = Player.World.GetBlock(pos);
            Debug.WriteLine("chunk - {0} , state: {1} , position: {2}, block: {3}",
                (chunk == null ? "null" : chunk.ToString()), (chunk == null ? "" : "" + chunk.State), pos, block.Name);
        }
    }
}