...
float2 CalculateAnimatedCoords(float2 coards)
{
    float2 result = {coards.x, coards.y};

    int i = 0;

    while (i < AnimationFrame){
        result.x += TextureStep;

        if (result.x >= 1){
            result.x = 0;
            result.y += TextureStep;
        }
        i++;
    }
...
VSOutput VertexShaderCommon(VSInput input, float4x4 instanceTransform)
{
    ...
    output.TexCoords1 = CalculateAnimatedCoords(input.TexCoords1);
